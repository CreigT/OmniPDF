      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
        <div className="grid grid-cols-2 md:grid-cols-5 gap-8 mb-12">
          ...
          <div className="col-span-2 space-y-4">
            ...
            <div className="inline-flex items-center px-3 py-1.5 rounded-full bg-rose-500/10 border border-rose-500/25 text-[11px] font-bold tracking-wide text-rose-300 uppercase">
          ...
          {/* Conversions */}
          <div>
            <p className="text-xs font-bold text-slate-200 uppercase tracking-wider mb-4">Conversions</p>
            <ul className="space-y-2.5 text-xs">
              ...
              <li><button onClick={() => onSelectTool('pdf-to-word')} className="hover:text-rose-400 transition-colors cursor-pointer">PDF to Word (DOCX)</button></li>
              ...
              <li><button onClick={() => onSelectTool('image-converter')} className="hover:text-rose-400 transition-colors cursor-pointer">Image Format Converter</button></li>
            </ul>
          </div>

          {/* Security & SaaS */}
          <div>
            <p className="text-xs font-bold text-slate-200 uppercase tracking-wider mb-4">Security & Plans</p>
            <ul className="space-y-2.5 text-xs">
              <li><button onClick={() => onSelectTool('print-pdf')} className="hover:text-rose-400 transition-colors cursor-pointer">Printout & Prep Tool</button></li>
              ...
              <li>
                <button onClick={() => onNavigate('pricing')} className="hover:text-rose-400 transition-colors cursor-pointer flex items-center gap-1 text-amber-400">
                  <Sparkles className="w-3 h-3" /><span>Pro Plans ($9/mo)</span>
                </button>
              </li>
              {user?.role === 'admin' ? (
                <li>
                  <button onClick={() => onNavigate('admin')} className="hover:text-indigo-400 transition-colors cursor-pointer text-indigo-400 flex items-center gap-1">
                    <Shield className="w-3 h-3" /><span>Admin Control Panel</span>
                  </button>
                </li>
              ) : (
                <li>
                  <button onClick={() => onNavigate('admin')} className="text-slate-400 hover:text-slate-400 transition-colors cursor-pointer text-[11px] flex items-center gap-1">
                    <Lock className="w-2.5 h-2.5" /><span>Staff Portal</span>
                  </button>
                </li>
              )}
            </ul>
        ...
        <div className="pt-8 border-t border-slate-900 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-slate-400">
          <div className="text-center sm:text-left">
            ...
            <p className="mt-1 font-semibold text-slate-300">Sponsored by CREIGNIFICENT LLC.</p>
          </div>
